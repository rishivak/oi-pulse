...
export default function DashboardPage() {
  ...
  const [livePayload, setLivePayload] = useState<SnapshotCreatedPayload | null>(null);
  ...
  const { data: latestSnapshot } = useQuery({
    queryKey: queryKeys.latest(underlying, selectedExpiry ?? "", interval),
    queryFn: () => oiApi.latest(underlying, selectedExpiry!, interval),
    enabled: !!selectedExpiry,
  });

  // Live snapshot updates via SSE
  useSSE<SnapshotCreatedPayload>(
    "snapshot_created",
    useCallback(
      (msg: SSEMessage<SnapshotCreatedPayload>) => {
        if (
          msg.payload.underlying === underlying &&
          msg.payload.interval_min === interval
        ) {
          setLivePayload(msg.payload);
          qc.invalidateQueries({ queryKey: queryKeys.latest(underlying, selectedExpiry ?? "", interval) });
        }
      },
  ...
  return (
    <div className="flex flex-col gap-0">
      <DashboardHeader
        ...
        spotPrice={livePayload?.spot_price ?? latestSnapshot?.spot_price ?? null}
        selectedExpiry={selectedExpiry}
        intervalMin={interval}
        lastUpdateTs={livePayload?.bucket_ts ?? latestSnapshot?.bucket_ts ?? null}
      />
      ...
      <div className="p-4">
        <KeyMetricsGrid snapshot={latestSnapshot ?? null} livePayload={livePayload} />
      </div>
    </div>
  );
}
